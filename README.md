...
- **在线 mofish5**：支持邀请对局、自动匹配、战绩统计和透明悬浮棋盘。
...
- 工具窗口模块显隐配置
- mofish5 在线用户、邀请对局、自动匹配、棋盘落子、认输和战绩统计
- mofish5 昵称与玩家身份持久化，以及可配置透明度、支持 `Ctrl+Alt+5` 快速切换的置顶悬浮棋盘
- 状态栏行情与收益轮播
...
mofish5 模块通过 WebSocket 与独立服务通信；服务端负责在线用户、匹配和对局状态，并持久化玩家战绩。
...
├── server/gomoku-server/          mofish5 WebSocket 服务、管理页和 Docker 部署配置
